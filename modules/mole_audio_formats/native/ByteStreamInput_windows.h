...
namespace mole {
    ...
    namespace WindowsMediaFoundation {
        ...
        //==========================================================================
        /** IMFByteStream interface implementation with juce::InputStream.  */
        class InputByteStream : public IMFByteStream, public IMFAsyncCallback, public IMFAttributes
        {
            juce::InputStream* input = nullptr;
            ...
            InputByteStream() = delete;
            InputByteStream (juce::InputStream* stream,
                    const wchar_t* mimeType = nullptr,
                    const wchar_t* originName = nullptr,
                    bool usingNetwork = false)
                : input (stream), isRemote (usingNetwork)
            {
            ...
            STDMETHODIMP Invoke (IMFAsyncResult* result) override
            {
                ...
                if (SUCCEEDED (hr))
                {
                    ...
                    if (args)
                    {
                        ...
                        args->bytesRead = input->read (args->buffer, (int) args->bufferSize);
                    }
                    else
                    {
                        hr = E_UNEXPECTED;
            ...
            STDMETHODIMP BeginRead (BYTE* buffer, ULONG bufferSize, IMFAsyncCallback* callback, IUnknown *state) override
            {
            ...
            STDMETHODIMP BeginWrite (const BYTE*, ULONG, IMFAsyncCallback*, IUnknown*) override { return NotImplemented (__FUNCTION__); }
            ...
            STDMETHODIMP EndRead (IMFAsyncResult* result, ULONG* bytesRead) override
            {
                IUnknown* punk = nullptr;
            ...
            STDMETHODIMP EndWrite (IMFAsyncResult*, ULONG*) override { return NotImplemented (__FUNCTION__); }

            /** Clears any internal buffers used by the stream.  */
            STDMETHODIMP Flush() override { return NotImplemented (__FUNCTION__); }
            ...
            STDMETHODIMP GetCapabilities (DWORD* cap) override
            {
                if (isRemote)
                {
                    *cap = MFBYTESTREAM_IS_REMOTE | MFBYTESTREAM_IS_READABLE | MFBYTESTREAM_IS_SEEKABLE | MFBYTESTREAM_HAS_SLOW_SEEK;
                }
                else
                {
                    *cap = MFBYTESTREAM_IS_READABLE | MFBYTESTREAM_IS_SEEKABLE;
                }
            ...
            {
                *len = (QWORD) input->getTotalLength();
                return S_OK;
            }
            ...
            {
                *isEnd = input->isExhausted();
                return S_OK;
            }

            /** Reads data from the stream.  */
            STDMETHODIMP Read (BYTE* buffer, ULONG bufferSize, ULONG* bytesRead) override
            {
                const juce::ScopedLock lock (criticalSection);

                *bytesRead = input->read (buffer, (int) bufferSize);
                return S_OK;
            }
            ...
            STDMETHODIMP Seek (MFBYTESTREAM_SEEK_ORIGIN mso, LONGLONG offset, DWORD /*flags*/, QWORD* currentPosition) override
            {
                ...
                if (mso == msoCurrent)
                    offset += (LONGLONG) input->getPosition();

                if (offset >= 0 && (juce::int64) offset <= input->getTotalLength())
                {
                    if (input->setPosition ((juce::int64) offset))
                    {
                        hr = S_OK;
                    ...
                    {
                        *currentPosition = (QWORD) input->getPosition();
                    }
                }
            ...
            {
                if (input->setPosition ((juce::int64) pos))
                    return S_OK;
            ...
            STDMETHODIMP Write (const BYTE*, ULONG, ULONG*) override { return NotImplemented (__FUNCTION__); }
            /// @}
            ...
            class AsyncArguments : public IUnknown
            {
                ...
                AsyncArguments (BYTE* buf, ULONG size) : buffer (buf), bufferSize (size) {}
                ...
                static AsyncArguments* Create (BYTE* buffer, ULONG bufferSize)
                {
            ...
            JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (InputByteStream);
        };
    } // namespace WindowsMediaFoundation
