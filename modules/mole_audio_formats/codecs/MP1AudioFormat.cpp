...
namespace mole {
    ...
    bool MP1AudioFormat::isChannelLayoutSupported (const juce::AudioChannelSet& channelSet)
    {
        return isChannelTagSupported (FormatTag::MP1, channelSet);
    }

    //=================================================================
    /* Tries to create an object that can read from a stream containing audio data in this format. */
    juce::AudioFormatReader* MP1AudioFormat::createReaderFor (
            juce::InputStream* sourceStream, bool deleteStreamIfOpeningFails)
    {
        std::unique_ptr<juce::AudioFormatReader> p (
                Codec().createDecoderFor (MediaFormat::MP1, sourceStream));
    ...
    std::unique_ptr<juce::AudioFormatWriter> MP1AudioFormat::createWriterFor (
            ...
            const juce::AudioFormatWriterOptions& options)
    {
        ...
        std::unique_ptr<juce::AudioFormatWriter> p (
                Codec().createEncoderFor (MediaFormat::MP1, streamToWriteTo, kbps, chan, rate, bits));
