...
namespace mole {
    ...
    bool FLACAudioFormat::isChannelLayoutSupported (const juce::AudioChannelSet& channelSet)
    {
        return isChannelTagSupported (FormatTag::FLAC, channelSet);
    }

    //=================================================================
    /* Tries to create an object that can read from a stream containing audio data in this format. */
    juce::AudioFormatReader* FLACAudioFormat::createReaderFor (
            juce::InputStream* sourceStream, bool deleteStreamIfOpeningFails)
    {
        std::unique_ptr<juce::AudioFormatReader> p (
                Codec().createDecoderFor (MediaFormat::FLAC, sourceStream));
    ...
    std::unique_ptr<juce::AudioFormatWriter> FLACAudioFormat::createWriterFor (
            ...
            const juce::AudioFormatWriterOptions& options)
    {
        ...
        std::unique_ptr<juce::AudioFormatWriter> p (
                Codec().createEncoderFor (MediaFormat::FLAC, streamToWriteTo, kbps, chan, rate, bits));
