...
namespace mole {
    ...
    bool MP2AudioFormat::isChannelLayoutSupported (const juce::AudioChannelSet& channelSet)
    {
        return isChannelTagSupported (FormatTag::MP2, channelSet);
    }

    //=================================================================
    /* Tries to create an object that can read from a stream containing audio data in this format. */
    juce::AudioFormatReader* MP2AudioFormat::createReaderFor (
            juce::InputStream* sourceStream, bool deleteStreamIfOpeningFails)
    {
        std::unique_ptr<juce::AudioFormatReader> p (
                Codec().createDecoderFor (MediaFormat::MP2, sourceStream));
    ...
    std::unique_ptr<juce::AudioFormatWriter> MP2AudioFormat::createWriterFor (
            std::unique_ptr<juce::OutputStream>& streamToWriteTo,
            const juce::AudioFormatWriterOptions& options)
    {
        UINT32 kbps = 0, bits = 0, chan = 0, rate = 0;
        ...
        {
            case 0:
                kbps = (rate < 32000) ? 160 : (chan == 1) ? 192 : 224;
                break;
            default:
        ...
        std::unique_ptr<juce::AudioFormatWriter> p (
                Codec().createEncoderFor (MediaFormat::MP2, streamToWriteTo, kbps, chan, rate, bits));
