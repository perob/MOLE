...
namespace mole {
    ...
    bool WMALosslessAudioFormat::isChannelLayoutSupported (const juce::AudioChannelSet& channelSet)
    {
        return isChannelTagSupported (FormatTag::WMALossless, channelSet);
    }

    //=================================================================
    /* Tries to create an object that can read from a stream containing audio data in this format. */
    juce::AudioFormatReader* WMALosslessAudioFormat::createReaderFor (
            juce::InputStream* sourceStream, bool deleteStreamIfOpeningFails)
    {
        std::unique_ptr<juce::AudioFormatReader> p (
                Codec().createDecoderFor (MediaFormat::WMALossless, sourceStream));
    ...
    std::unique_ptr<juce::AudioFormatWriter> WMALosslessAudioFormat::createWriterFor (
            ...
            const juce::AudioFormatWriterOptions& options)
    {
        ...
        std::unique_ptr<juce::AudioFormatWriter> p (
                Codec().createEncoderFor (MediaFormat::WMALossless, streamToWriteTo, kbps, chan, rate, bits, vbrq));
